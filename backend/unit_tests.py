...
def test_add_class():
    response = client.post(
        "/model/classes/add", params={"label": "test", "number_of_images": 5}
    )
    assert response.status_code == 200
...
def test_update_class():
    response = client.post(
        "/model/classes/update", params={"oldlabel": "test", "newlabel": "test2"}
    )
    assert response.status_code == 200
...
def test_evaluate_model():
    response = client.get("/model/evaluate")
    assert response.status_code == 200
    assert response.json() == {"message": "Model evaluated successfully"}


def test_predict():
    response = client.post("/model/predict", files={"file": open("test.jpg", "rb")})
    assert response.status_code == 200
    assert response.json() == {"message": "Predicted successfully"}
